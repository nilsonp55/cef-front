...
@Component({
  selector: 'app-ventana-emergente-eliminar-rechazar-t',
  templateUrl: './ventana-emergente-eliminar-rechazar-t.component.html',
  styleUrls: ['./ventana-emergente-eliminar-rechazar-t.component.css']
})
export class VentanaEmergenteEliminarRechazarTComponent implements OnInit {

  observacion: any = "";
  accion: any;

  constructor(@Inject(MAT_DIALOG_DATA) public data: any, private dialog: MatDialog, private dialogRef: MatDialogRef<VentanaEmergenteEliminarRechazarTComponent>,
  ...
  aceptarRechazar() {
    if (this.observacion.length < 1) {
      ...
      this.opConciliacionCostosService.eliminarRechazarLiquidadasNoCobradasTransporte({
        ...
        next: (response: any) => {
          if (response.response.description === "Success") {
            let accion = this.data.msn.texto == "Eliminar" ? "eliminados" : "rechazados"
