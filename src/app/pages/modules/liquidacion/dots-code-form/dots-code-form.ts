...
@Component({
  selector: 'app-dots-code-form',
  templateUrl: './dots-code-form.html',
  styleUrls: ['./dots-code-form.css']
})
export class DotsCodeFormComponent implements OnInit {

  @Input() initialData: any; // For editing
  @Input() bancos: any[] = [];
  @Input() transportadoras: any[] = [];
  @Input() ciudades: any[] = [];
  // clientes might be loaded internally or passed as Input
  // puntos might be loaded internally based on selections, or passed as Input
  ...
  // Internal lists, might be populated from inputs or fetched
  puntos: any[] = [];
  clientes: any[] = [];
  ...
  constructor(
    private fb: FormBuilder, // Using FormBuilder for cleaner form creation
  ...
  async initForm(param?: any) {
    ...
    if(param) {
      ...
      if(param.ciudadFondo && this.ciudades.length > 0) { // Ensure ciudades is populated
    ...
    this.form = this.fb.group({
      ...
      'cliente': [param ? clienteValueForm : null], // Might need adjustment
  ...
  async onSubmit() {
    ...
    this.spinnerActive = true;
    const formData = this.form.getRawValue(); // Use getRawValue to include disabled controls like tipoPunto
  ...
  async changeBanco(event: any) {
    if (!this.selectedTipoPunto) return; // Ensure tipoPunto is selected
    ...
    this.form.controls['codigoPunto'].setValue(''); // Reset codigoPunto hidden field
    this.spinnerActive = false;
  }
