...
export class PreliminarGestionCargueDefinitivoGuard implements CanActivate {
    ...
    return new Promise((resolve, reject) => {
      this.rolMenuService.obtenerUsuarios({
        ...
        this.rolMenuService.obtenerMenuRol({
          ...
          menusrol.data.forEach(itm => {
            if (itm.menu.url == "administrador-cargue-definitivo") {
              guardOk = true;
            }
          });
          if (guardOk) {
            ...
            this.router.navigate(['/cargue-programacion']);
            resolve(false);
          }
        });
      })
